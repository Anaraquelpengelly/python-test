...
def csrf_lab_login(request):
    if request.method == 'GET':
        ...
        ngo.contrib.auth.hashers import check_password
        try:
            user_obj = CSRF_user_tbl.objects.get(username=username)
            if check_password(password, user_obj.password):
                User = CSRF_user_tbl.objects.filter(pk=user_obj.pk)
            else:
                User = CSRF_user_tbl.objects.none()
