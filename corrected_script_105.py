...
# Add BasicConstraints extension, crucial for end-entity certificates
cert = cert.add_extension(
    x509.BasicConstraints(ca=False, path_length=None), critical=True,
)

# Copy extensions from the CSR to the certificate
# This ensures extensions like Subject Alternative Name (SAN) are included
for extension in csr.extensions:
    # Avoid adding BasicConstraints again if it was in the CSR, as we explicitly set it
...
with tempfile.NamedTemporaryFile(mode='wb', prefix='acme_', suffix='.cert', dir='/tmp', delete=False) as out:
