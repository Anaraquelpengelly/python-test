...
if __name__ == '__main__':
    ...
    if request.method == 'POST':
        ...
        if len(password) < 8 or not any(char.isdigit() for char in password) or \
           not any(char.isalpha() for char in password) or not any(char in '!@#$%^&*()_+' for char in password):
    ...
    if request.method == 'POST':
        ...
        if found_user:
            ...
            password_reset_tokens[token] = {'username': found_user, 'expires': expires_at}
            flash(f'Password reset link: {url_for("reset_form", token=token, _external=True)}', 'info')
            return redirect(url_for('lab'))
    ...
    if request.method == 'POST':
        ...
        if len(new_password) < 8 or not any(char.isdigit() for char in new_password) or \
           not any(char.isalpha() for char in new_password) or not any(char in '!@#$%^&*()_+' for char in new_password):
            flash('New password must be at least 8 characters long and contain letters, numbers, and special characters.', 'danger')
            return render_template('reset.html', token=token)
