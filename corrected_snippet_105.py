#!/usr/bin/env python3

import datetime

from cryptography.hazmat.backends import default_backend
from cryptography.hazmat.primitives.asymmetric import rsa
from cryptography.hazmat.primitives import serialization
from cryptography import x509
from cryptography.x509.oid import NameOID
from cryptography.hazmat.primitives import hashes


with open("/tmp/ca.cert", "rb") as ca_cert_file:
    ca_cert = x509.load_pem_x509_certificate(ca_cert_file.read(), default_backend())

with open("/tmp/acme.csr", "rb") as csr_file:
    csr = x509.load_pem_x509_csr(csr_file.read(), default_backend())

with open("/tmp/ca.key", "rb") as key_file:
    private_key = serialization.load_pem_private_key(
        key_file.read(),
        password=None,
        backend=default_backend()
    )

cert = x509.CertificateBuilder().subject_name(csr.subject)
cert = cert.issuer_name(ca_cert.subject)
cert = cert.public_key(csr.public_key())
cert = cert.serial_number(x509.random_serial_number())
cert = cert.not_valid_before(datetime.datetime.utcnow())
cert = cert.not_valid_after(datetime.datetime.utcnow() + datetime.timedelta(days=30))
cert = cert.sign(private_key, hashes.SHA256(), default_backend())

# Write our certificate out to disk.
#!/usr/bin/env python3
import datetime
import tempfile
import os
import stat
from cryptography.hazmat.backends import default_backend
from cryptography.hazmat.primitives.asymmetric import rsa
from cryptography.hazmat.primitives import serialization
from cryptography import x509
from cryptography.x509.oid import NameOID
from cryptography.hazmat.primitives import hashes
with open("/tmp/ca.cert", "rb") as ca_cert_file:
    ca_cert = x509.load_pem_x509_certificate(ca_cert_file.read(), default_backend())
with open("/tmp/acme.csr", "rb") as csr_file:
    csr = x509.load_pem_x509_csr(csr_file.read(), default_backend())
key_path = "/tmp/ca.key"
if not os.path.exists(key_path): raise FileNotFoundError(f"File not found: {key_path}")
key_stat = os.stat(key_path)
if bool(key_stat.st_mode & stat.S_IWOTH) or bool(key_stat.st_mode & stat.S_IWGRP):
    raise PermissionError(f"Insecure file permissions for {key_path}. It should not be world or group writable.")
if os.path.islink(key_path):
    raise ValueError(f"File {key_path} is a symbolic link, which is not allowed.")
with open(key_path, "rb") as key_file:
    private_key = serialization.load_pem_private_key(
        key_file.read(),
        password=None,
        backend=default_backend()
    )
cert = x509.CertificateBuilder().subject_name(csr.subject)
cert = cert.issuer_name(ca_cert.subject)
cert = cert.public_key(csr.public_key())
cert = cert.serial_number(x509.random_serial_number())
cert = cert.not_valid_before(datetime.datetime.utcnow())
cert = cert.not_valid_after(datetime.datetime.utcnow() + datetime.timedelta(days=30))
cert = cert.sign(private_key, hashes.SHA256(), default_backend())
# Write our certificate out to disk.
fd, temp_path = tempfile.mkstemp(suffix='.cert', prefix='acme_', dir='/tmp')
try:
    with os.fdopen(fd, 'wb') as out:
        out.write(cert.public_bytes(serialization.Encoding.PEM))
    os.rename(temp_path, '/tmp/acme.cert')
except:
    os.unlink(temp_path)
    raise
print('Created /tmp/acme.cert')
    out.write(cert.public_bytes(serialization.Encoding.PEM))

print('Created /tmp/acme.cert')

