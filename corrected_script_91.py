...
def xss_lab2(request):
    if request.user.is_authenticated:
        ...
        if not username:
            username = "Guest"
        # Manual string replacement for XSS prevention is ineffective and should be avoided.
        # Django's template engine auto-escapes variables by default, which is the correct way to prevent XSS.
        context = {
        'username': username
                }
